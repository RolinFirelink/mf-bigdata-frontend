 ...
 * @LastEditTime: 2023-08-18 14:35:00
 * @FilePath: \mf-bigdata-frontend\src\views\data\product-market\ProductMarketModal.vue
...
<script lang="ts">
  ...
  import { productMarketFormSchema } from "./productMarket.data";
  import { BasicModal, useModalInner } from "/@/components/general/Modal";
  import { insertProductMarket, updateProductMarket } from "/@/api/data/ProductMarket";
  import { listRegionByPid, getRegionById } from "/@/api/sys/Region";

  export default {
    name: "ProductMarketModal",
    ...
    setup(_, { emit }) {
      ...
      const [registerForm, { resetFields, setFieldsValue, validate, updateSchema }] = useForm({
        ...
        schemas: productMarketFormSchema,
      ...
      const getTitle = computed(() => (!unref(isUpdate) ? "新增批发市场表" : "编辑批发市场表"));
      ...
      async function handleSubmit() {
        ...
        if (unref(isUpdate)) {
          saveProductMarket(updateProductMarket, values);
        } else {
          saveProductMarket(insertProductMarket, values);
        }
      }

      function saveProductMarket(save, values) {
        save(values)
          .then(() => {
            emit("success");
            closeModal();
